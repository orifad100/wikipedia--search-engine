...
def partition_postings_and_write(postings):
    ...
    return postings.map(lambda x: (token2bucket_id(x[0]), (x[0], x[1]))).groupByKey().map(
...
DL = {}
nf = {}
doc_id_counts = postings_filtered.flatMap(lambda x: [(doc_id, tf) for doc_id, tf in x[1]]).reduceByKey(
    lambda x, y: x + y)
